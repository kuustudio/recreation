$(function(){
	...
	$('#switch-znz-bet').toggle(function(){
		...
		$('#znz-game').load('/index/znz/'+game.type, function(){
			$('#bet-game').hide();
			$(this).show();
		});
	},function(){
	...
	$('#znz-code-list .qzbtn').live('click', function(){
		...
		$('<div><p>最大中奖积分：<span class="zj-amount"></span>积分</p><p>支付返点：<span class="fan-dian"></span>积分</p><p>需要冻结资金共：<span class="dj-amount"></span>积分</p></div>')//<p>支付抽水：<span class="shou-shui">积分</span></p>
					...
					$.ajax({
						...
						success:function(data, textStatus, xhr){
							...
							if(errorMessage){
								...
								$('#znz-game').load('/index/znz/'+game.type);
								alert(data);
								reloadMemberInfo();
							}
						}
