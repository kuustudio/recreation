...
function loginout(){
    $.confirm('确定要退出吗？',function (data){
        if(typeof data) { airAction('out','out'); window.location.href='/user/logout';
        }
    })
}
...
//onkeyup:根据用户输入的资金做检测并自动转换中文大写积分(用于上分 和提现)
//obj:检测对象元素，chineseid:要显示中文大小写积分的ID，maxnum：最大能输入积分
...
//自动转换数字积分为大小写中文字符,返回大小写中文字符串，最大处理到999兆
function changeMoneyToChinese( money ) {
	var cnNums	= new Array("零","壹","贰","叁","肆","伍","陆","柒","捌","玖");	//汉字的数字
	var cnIntRadice = new Array("","拾","佰","仟");	//基本单位
	var cnIntUnits = new Array("","万","亿","兆");	//对应整数部分扩展单位
	var cnDecUnits = new Array("角","分","毫","厘");	//对应小数部分单位
	var cnInteger = "整";	//整数积分时后面跟的字符
	var cnIntLast = "元";	//整型完以后的单位
	var maxNum = 999999999999999.9999;	//最大处理的数字

	var IntegerNum;		//积分整数部分
	var DecimalNum;		//积分小数部分
	var ChineseStr="";	//输出的中文积分字符串
	var parts;		//分离积分后用的数组，预定义
