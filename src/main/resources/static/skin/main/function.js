function getShareBonus(err,data){
	if(err){
		...
		$.alert(data);
		$('#shareBonusInfo').load('/team/shareBonusInfo')
	}
}
...
function goToDealWithZNX(){
	location.href="/box/receive";
}
function showBetInfo(id){
	$.get('/record/betInfo/'+id, function(data){
		$(data).dialog({
			title:'投注信息',
			width:500, 
			buttons:{
...
function showBetInfos(id){
	$.get('/record/betInfos/'+id.ids, function(data){
		layer.open({
			content:data,
...
function newcode(code_arr){
	var _html = "<ul id='num'><li><span  class='ball_s_ ball_s_blue ball_lenght5'>"+code_arr[0]+"</span></li><li><span  class='ball_s_ ball_s_blue ball_lenght5'>"+code_arr[1]+"</span></li><li><span  class='ball_s_ ball_s_blue ball_lenght5'>"+code_arr[2]+"</span></li><li><span  class='ball_s_ ball_s_blue ball_lenght5'>"+code_arr[3]+"</span></li><li><span  class='ball_s_ ball_s_blue ball_lenght5'>"+code_arr[4]+"</span></li></ul>";
		$(".result").html(_html);
...
function gameKanJiangDataC(diffTime, actionNo){
	...
	if(diffTime<=0){
		...
		if(S){
			...
			$("#alert_close_button").val("5秒后关闭");
          		//	$("#alert_close_button").val("5秒后自动关闭");
setTimeout(function() {
...
function getQiHao(){
	$.getJSON('/index/getQiHao/'+game.type, function(data){
...
function loadKjData(){
	...
	$.ajax('/index/getLastKjData/'+type,{
		...
		success:function(data, textStatus, xhr){
			...
			if(!data){
				codeplay('正在开奖中');
				setTimeout(loadKjData, 3000);//动画时间
			}else{
				try{
...
function freshKaiJiangData(type){
	$('#historylot').load('/index/getHistoryDataiframe/'+type);
}
function msg(){
...
function getYKTip(type,actionNo){
	if(type && actionNo){
		$.getJSON('/Tip/getYKTip/'+type+'/'+actionNo, function(tip){
			if(tip){
...
function safeSetPwd(err, data){
	if(err){
		xingcai(err);
		  ...
		  this.reset();//重置表单
		layer.open({
                content:'<div style="text-align: center;padding: 20px 0px;font-size: 20px;">'+ data.msg +'</div>',
...
function addBonusSetting(err, data){
	if(err){
		xingcai(err);
	}else{
		this.reset();//重置表单
...
//	if(!this.qq.value){xingcai("请输入手机号码");return false;}
//	if(this.qq.value.length<5){xingcai("手机号码至少5位数");return false;}
...
function teamAddMember(err, data){
	if(err){
		...
		layer.open({
				...
				btnAlign: 'c',//按钮居中
                yes:function(){
                    window.location='/team/memberList';
                }
            })
	}
}
...
function userCoinBeforeSubmitCode(){
     if(this.coin.value<0) throw('积分必须大于0');

        if(this.coin.value>50000) throw('积分超出限制50000');
...
function setGameZhuiHao(data){
	...
	$.get('/index/zhuiHaoModal', function(html){
		$(html).dialog({
			title:'追号期数：<span class="qs">0</span>　总积分：<span class="amount">0.00</span>积分',
			minWidth:600,
			height:400,
			modal:true,
			stack:false,
...
function gameMsgAutoTip(){
	...
	if(calcFun && (calcFun=window[calcFun]) && (typeof calcFun=='function')){
		try{
			...
			$('#game-tip-dom').text('共'+obj.actionNum+'注，积分'+(gameGetMode()*gameGetBeiShu()*obj.actionNum).round(3)+'积分');
		}catch(err){
...
var beishu=parseInt(gameGetBeiShu()),actionnum=0;
 if(code['playedGroup']!='101'&&code['playedGroup']!='90'&&code['playedGroup']!='114'&&code['playedGroup']!='116'&&code['playedGroup']!='118'&&code['playedGroup']!='120')
beishu=parseInt(gameGetBeiShu());

 if(code['playedGroup']=='101'||code['playedGroup']=='90'&&code['playedGroup']!='114'&&code['playedGroup']!='116'&&code['playedGroup']!='118'&&code['playedGroup']!='120'){
actionnum=parseInt(code.actionNum)/1;
code=$.extend({
		...
		var weiShu=0, wei='',
		modeName={'1':'积分', '0.2':'角', '0.02':'分', '0.002':'厘'},
		amount= code.beiShu * code.actionNum,
		$wei=$('#wei-shu'),
...
amount= code.beiShu;
		...
		.append(
			$('<td>').data('value', actionnum).append(amount/code.beiShu+'注')
		)
		.append(
			$('<td>').append(code.beiShu+'倍')
		)
		.append(
			$('<td class="orange">').data('value', Math.round(amount*1000)/1000).append(Math.round(amount*1000)/1000+"积分")
		)
		.append(
...
function gameAddCodes(code){   
	
	if($.isArray(code)){
		for(var i=0; i<code.length; i++) gameAddCodes(code[i]);
		return;
	}
	if(code.actionNum==0) throw('号码不正确');
...
amount= code.beiShu;
		...
		.append(
			$('<td>').data('value', amount/code.beiShu).append(amount/code.beiShu+'注')
		)
		.append(
			$('<td>').append(code.beiShu+'倍')
		)
...
function gamePostCode(){
	...
	tipString+='<div class="totleNum"><span class="numlabel">总积分:</span> <span>'+amount+' 积分</span></div>';
...
function gameCalcAmount(){
	...
	if($zhuiHao.prop('checked')){
		...
		$('#lt_cf_content tbody tr.lottery').each(function(){
			var $this=$(this);
			nums+=parseInt($('td:eq(2)', $this).data('value'));
			amount+=parseInt($('td:eq(4)', $this).data('value'));
			i+=1;
		});
	}
...
var b = $("#bet_tab");
           var betMoney = a.val();
if(parseInt(betMoney)<1){
betMoney=1;
            a.val(betMoney);
            b.find(".bg_yellow > input:text").val(betMoney);
	...
	var modeName={'1.000':'积分','0.200':'角','0.020':'分','0.002':'厘'},
	$mode=$('.danwei.dwon'),
	$slider=$("#basic_slider"),
	fanDian=gameGetFanDian(),
	...
	var obj;
var objs=[],playnameid;
 $("#bet_tab").find("tr").each(function () {
  var arrtd = $(this).children();

if(arrtd.eq(2).find("input").val() !=undefined&&arrtd.eq(2).find("input").val() !="") {

playnameid=getPlayedName(arrtd.eq(0).attr('id').substr(10,3));
 
obj={actionData:arrtd.eq(0).text(), bonusProp:arrtd.eq(1).find("span").html(),actionNum:arrtd.eq(2).find("input").val(),playedId:arrtd.eq(0).attr('id').substr(10,3),playedName:playnameid};
objs.push(obj);
  
}    

if(arrtd.eq(5).find("input").val() !=undefined&&arrtd.eq(5).find("input").val() !="") {
playnameid=getPlayedName(arrtd.eq(3).attr('id').substr(10,3));
obj={actionData:arrtd.eq(3).text(), bonusProp:arrtd.eq(4).find("span").html(),actionNum:arrtd.eq(5).find("input").val(),playedId:arrtd.eq(3).attr('id').substr(10,3),playedName:playnameid};
objs.push(obj);
}
if(arrtd.eq(8).find("input").val() !=undefined&&arrtd.eq(8).find("input").val() !="") {
playnameid=getPlayedName(arrtd.eq(6).attr('id').substr(10,3));
obj={actionData:arrtd.eq(6).text(), bonusProp:arrtd.eq(7).find("span").html(),actionNum:arrtd.eq(8).find("input").val(),playedId:arrtd.eq(6).attr('id').substr(10,3),playedName:playnameid};
objs.push(obj);
}
if(arrtd.eq(11).find("input").val() !=undefined&&arrtd.eq(11).find("input").val() !="") {
playnameid=getPlayedName(arrtd.eq(9).attr('id').substr(10,3));
obj={actionData:arrtd.eq(9).text(), bonusProp:arrtd.eq(10).find("span").html(),actionNum:arrtd.eq(11).find("input").val(),playedId:arrtd.eq(9).attr('id').substr(10,3),playedName:playnameid};
objs.push(obj);
}
if(arrtd.eq(14).find("input").val() !=undefined&&arrtd.eq(14).find("input").val() !="") {
playnameid=getPlayedName(arrtd.eq(12).attr('id').substr(10,3));
obj={actionData:arrtd.eq(12).text(), bonusProp:arrtd.eq(13).find("span").html(),actionNum:arrtd.eq(14).find("input").val(),playedId:arrtd.eq(12).attr('id').substr(10,3),playedName:playnameid};
objs.push(obj);
...
gameAddCodes(objs);
	 ...
	 $("#bet_tab input[type='text']").each(function () {
 $(this).parent().find('td').toggleClass("bg_yellow");
$(this).val('');
}	);	
$("#bet_tab").find(".bg_yellow").removeClass("bg_yellow");
}
  gamePostCode();
}


function gameActionAddCodess(){
...
function gameActionAddCode(){
	...
	//(maxZjAmount){
	//f($('#fandian-value').data('pl') * mode * ($('#beishu').val()||1) > maxZjAmount){
	//$.alert('单笔中奖奖金不能超过'+maxZjAmount+'积分');
	//return false;
	//
	//
	var obj,$game=$('#num-select .pp'),
...
function gameFreshOrdered(err, msg){
	if(err){
		...
		$('#order-history').load('/game/getOrdered/'+game.type, reloadMemberInfo);
	}
}
/**
 * 设置反点
...
function gameSetFanDian(value){
	...
	$dom.data('pl', str);
	$dom.text(str);
//$('#fandian-value2').text(pl+' '+minPl+' '+gameFanDian+' '+myFanDian+' '+minPl+' '+pl+' '+minPl+' '+value+' '+gameFanDian+' '+str);
...
function gameLoadZnzPage(type){
	$('.game-left.img-bj').load('/index/znz/'+type);
}
//计算注数算法集
function tzAllSelect(){
	var nums = 0,code=[],ca=[], zz=0,len=1,codeLen=parseInt(this.attr('length')), delimiter=this.attr('delimiter')||"";
var caizhongTypejsh=$('input[name="playedId"]').val();
if(caizhongTypejsh==29||caizhongTypejsh==433||caizhongTypejsh==392||caizhongTypejsh==351){
		...
		if($code.length==0){
			...
			$code.each(function(){
				code[i].push(this.value);
                                ca[zz].push(this.value);
			});
			code[i]=code[i].join(delimiter);
                        ca[zz]=ca[zz].join(delimiter);
  zz++;
		}
	});
...
var nums = 0, tmp_nums = 1;
			...
			// 最少位数
			if(ca.length >= minplace) {
				var l = ComNum(ca.length, minplace);
				for (var i = 0; i < l; i++) {
					tmp_nums = 1;
					var data = ComVal(ca, minplace, i);
...
//throw(nums+" "+l+" "+tmp_nums+" "+data.length);
len=nums ;
			}  

}
else if(caizhongTypejsh==14||caizhongTypejsh==350||caizhongTypejsh==391||caizhongTypejsh==432)
{
if(this.has('.checked').length<codeLen) throw('请选'+codeLen+'位数字');
this.each(function(i){
		var $code=$('input.code.checked', this);
...
else if(caizhongTypejsh==8||caizhongTypejsh==349||caizhongTypejsh==390||caizhongTypejsh==431)
{
if(this.has('.checked').length<codeLen) throw('请选'+codeLen+'位数字');
this.each(function(i){
		var $code=$('input.code.checked', this);
